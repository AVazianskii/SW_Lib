...
namespace SW_Character_creation
{
    public partial class Age_status_manager
    {
        public void Run_download_and_upload_data()
        {
            ...
            Run_download_from_SQLite("SELECT * FROM Age_statuses_general_info ORDER BY ID",
                                     SQLite_connection,
                                     Age_status_general_coloumn_name,
                                     Age_status_general_description,
                                     (int)Type_of_var.string_type);

            Run_download_from_SQLite("SELECT * FROM Age_statuses_attributes_bonus ORDER BY ID",
                                     SQLite_connection,
                                     Age_status_bonus_coloumn_name,
                                     Age_status_attributes_bonus,
                                     (int)Type_of_var.int_type);

            Run_download_from_SQLite("SELECT * FROM Age_statuses_attributes_limits ORDER BY ID",
                                     SQLite_connection,
                                     Age_status_attributes_limits_coloumn_name,
                                     Age_status_attributes_limits,
                                     (int)Type_of_var.int_type);

            Run_download_from_SQLite("SELECT * FROM Age_statuses_skills_limits ORDER BY ID",
                                     SQLite_connection,
